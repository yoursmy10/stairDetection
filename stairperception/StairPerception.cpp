...
bool StairDetection::process(const pcl::PointCloud<PointType> &cloud_in,
            ...
            pcl::PointCloud<PointType> &addon_cloud)
{
...
//          rotationCloudPoints(cloud_in,rotation_matrix,cloud_rotation,4);
        ...
        calc_stair_direction(stair, addon_cloud);
...
void StairDetection::computePlaneCounter(Stair &stair)
{
    ...
    while(true)
    {
        if(pnext->pnext_type == step_point)
        {
            ...
            {
                // 台阶垂直面上边沿直线参数   xyz+ direction:xyz
...
void StairDetection::calc_stair_direction(Stair &stair, CloudType &addon_cloud)
{
    //1. calculate and store the mid-center point
    /*
    Node *pnext;
    pnext = stair.getHead();
    pcl::PointXYZRGBA point;
    pcl::PointXYZ mid_center_point;
    ...
    while(pnext->pnext != nullptr){
        if(pnext->pnext_type == step_point){
                ...
                //get RGBA
                //GET XYZ
                point.x = (pstep->plane_v->counter.points[2].x)/2 + (pstep->plane_v->counter.points[3].x)/2;
                point.y = (pstep->plane_v->counter.points[2].y)/2 + (pstep->plane_v->counter.points[3].y)/2;
                point.z = (pstep->plane_v->counter.points[2].z)/2 + (pstep->plane_v->counter.points[3].z)/2;
                //push_back the line points
                addon_cloud.push_back(point);
                std::cout<<"point "<<"x:"<<point.x<<"\n"<<"y: "<<point.y<<"\n"<<"z: "<<point.z<<std::endl;
        }
        pnext = pnext->pnext;
    }
    //2. linealize by points   store the start point(counter == 0) and the line's direction
    
    pnext = stair.getHead();  
    */
    //3. store the height of each step

    //4. draw the line and Point in vtkweidget
    //using addLIne to draw line using two points
} 