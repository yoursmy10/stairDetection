...
private:
    ...
    std::vector<PointType> line_points;
...
public:
        /*
        cloud_in:输入的点云
        show_mode:显示模式，他的算法是根据显示模式决定运行到哪一步的
        stair：楼梯模型
        vector_plane_sorted：收集被分割出的平面，根据showmode决定输出到哪一个步骤的平面集合
        cloud_show：点云，输出类似于vector_plane_sorted
        normal_show：输出法线
        */
...
private:
    ...
    void calc_stair_direction(Stair &stair, CloudType &addon_cloud);
